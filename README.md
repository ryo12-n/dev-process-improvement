...
起動すると、L1マネージャーがバックログから新施策を開始するか、既存施策を継続するかを質問する。
回答時に対象のパスを貼り付ける。

- **新施策を開始する場合**: `backlog/<施策名>.md` のパスを指定
- **既存施策を継続する場合**: `sessions/initiatives/<フォルダ名>/` のパスを指定
...
リポジトリ間（dev-process-improvement → ai-driven-dev-patterns 等）で構成・ルール・テンプレートを同期するセッション。
同期作業の実施と、7つの検証観点による品質チェックを構造化して行う。
...
起動すると、同期先リポジトリと同期方向を質問される。

- **入力形式**: `sync: <target-repo> [<direction>]`
- **例**: `sync: ai-driven-dev-patterns origin-to-upstream`

> - sync-worker / sync-evaluator はマネージャーが自律的に起動する。
> - 同期作業は [`repo-sync-checklist`](.claude/skills/repo-sync-checklist/SKILL.md) に従い、評価は7つの検証観点で実施される。
