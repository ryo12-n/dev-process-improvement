...
- **種別**: 知見
- **内容**: `01_plan.md` および `02_tasks.md` で成果物パスを `docs/design/dev-workflow-overview.md` と記載しているが、このパスが `` からの相対パスなのか、リポジトリルートからの相対パスなのかが曖昧。実際のファイルはリポジトリルートの `docs/design/dev-workflow-overview.md` に配置されている。`docs/design/` ディレクトリは存在しない
