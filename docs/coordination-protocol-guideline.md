...
> **文書ステータス**: ドラフト（参考資料）
> **正の情報源への統合**: 一部統合済み（セクション2.2 → `.claude/rules/commit-message.md`）
> Skills 統合後に削除を検討する。
...
| `initiative` | 施策名（sessions/initiatives/ 配下のディレクトリ名） | `coordination-protocol` |
...
resource: sessions/initiatives/coordination-protocol/07_issues.md
```
...
**作成者**: L2（実施）
**作成日**: 2026-02-25
**関連施策**: `sessions/initiatives/coordination-protocol/`
**参照資料**:
- `refs/ai-driven-development-poc/` (並列 Claude 開発フレームワーク)
- `docs/git-worktree-guideline.md` (git worktree ガイドライン)
- `sessions/initiatives/coordination-protocol/00_proposal.md` (本施策の提案書)
