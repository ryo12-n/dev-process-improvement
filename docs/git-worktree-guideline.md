...
## 7. impl-manager ハイブリッド方式

> **正の情報源**: `.claude/skills/l1-impl-manager/SKILL.md`（並列ディスパッチ機構セクション）

### 設計判断

impl-manager の Phase 4 並列ディスパッチでは、2つの異なるリポジトリにまたがる競合防止が必要:

| リポジトリ | 競合の種類 | 対策 |
|-----------|-----------|------|
| 外部リポジトリ（コードファイル） | 複数ワーカーの同一ファイル変更 | Git worktree で物理分離 |
| dev-process-improvement（セッションファイル） | `07_issues.md` への同時書き込み | per-worker ファイル分離（`07_issues_W<N>.md`） |

この2つの対策を組み合わせた「ハイブリッド方式」を採用している。

### ブランチ命名規則

- impl-manager 固有: `impl/<施策名>`（通常の L1 initiatives の `initiative/` とは区別）
- per-worker ブランチ: `impl/<施策名>-W<N>`（worktree ごとに分離）

### 3層検証チェックリスト

フェールセーフとして、3つのレイヤーで検証を行う:

| レイヤー | 実施者 | タイミング | 目的 |
|---------|--------|-----------|------|
| 事前検証 | マネージャー | Wave ディスパッチ前 | worktree・ファイル・ブランチの準備完了を保証 |
| ワーカーセルフチェック | ワーカー | 作業完了時 | 自己検証（忘れても問題ない） |
| 事後検証 | マネージャー | Wave 完了後 | 全項目の検証・不足分の回復 |

詳細な各チェックリストの項目は `.claude/skills/l1-impl-manager/SKILL.md` を参照。

### マージフロー

```
[事前準備]
  worktree 作成 → per-worker ファイル作成 → 事前検証チェックリスト
                                                    ↓
[Wave ディスパッチ]
  impl-worker W1 → worktree W1 で実装 → impl/<施策名>-W1 にコミット
  impl-worker W2 → worktree W2 で実装 → impl/<施策名>-W2 にコミット
                                                    ↓
[Wave 完了後処理]
  事後検証チェックリスト → per-worker ブランチを施策ブランチにマージ
  → マージ後テスト → 課題統合 → worktree クリーンアップ
```
