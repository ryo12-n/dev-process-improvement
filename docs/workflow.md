...
L1 マネージャーは以下3パターンの入力を受け付ける:

| パターン | 入力形式 | 動作 |
|---------|---------|------|
| 1 | `backlog: <施策名>` | backlog エントリから新施策を開始 |
| 2 | `施策: <パス>` | 既存施策フォルダの状態に応じて継続（worker 起動 / ゲート判定） |
| 3 | `project: <プロジェクト名>` | `projects/<名前>/todo.md` の未着手施策を一覧表示し、選択した施策を開始 |

パターン3は project の todo.md を読み、未着手施策を提示する。ユーザーが施策を選択すると、backlog エントリがあればパターン1に、なければ plan.md の情報を参照して壁打ちフェーズから開始する。
...
| 対象 | 適用条件 | やること | 完了条件 |
|------|---------|---------|---------|
| L2-worker | ルール・テンプレート変更施策 | メタルール横断検証（フロー記述・workflow.md・TG-008基準）を実施し結果を記録 | 3領域の検証結果が `phase-*/set-N/03_worker_report.md` に記載 |
| L2-worker | ルール変更施策 | `rule-change-checklist` スキルの全7項目を確認（deny リスト副作用分析チェックを含む） | 全7項目の確認結果が `phase-*/set-N/02_worker_log.md` に記録 |
| L2-worker | プロジェクト紐づけ施策 | `projects/<name>/todo.md` のステータス更新、`knowledge.md` に知見追記、`plan.md` のステータス更新 | todo.md・knowledge.md・plan.md の3ファイルが更新済み |

### ゲート判定での課題重大度判定

> **正の情報源**: `manager-common-policy` §4.1

ゲート判定で課題の扱いを決定する際、以下の3段階で重大度を判定する。

| 重大度 | 基準 | ゲート判定への影響 |
|--------|------|-----------------|
| ブロック課題 | 次フェーズの前提条件を満たせない | 差し戻し |
| 非ブロック課題（重要） | 次フェーズで対処可能 | 条件付き通過 |
| 非ブロック課題（軽微） | 施策スコープ外 | 通過（知見ルーティング） |

### リファレンス文書の粒度基準

> **正の情報源**: `manager-common-policy` §5.4

`refs/<リポジトリ名>/knowledge.md` への追記は、再利用性・発見コスト・制約/ハマりポイントの3基準で判断する。`work-logs/` は調査ステップが3以上または試行錯誤を含む場合に作成する。

### スキル/エージェント設計ガイド

> **正の情報源**: `session-flow-policy` §2.2, §2.5, §3.6, §3.7

- **description 設計**: `user-invocable: true` は動詞始まりで「何をするか」、`false` は「何の情報を持つか」+「どの文脈で参照するか」の2要素パターン
- **関連ファイル一覧**: SKILL.md・エージェント定義の末尾にテーブル形式で連動更新対象を列挙（`rule-change-checklist` §1 と連携）
- **エージェント管理**: `.claude/skills/<skill>/agents/` に配置（スキル固有）。グローバル配置（`.claude/agents/`）は複数スキル共通の場合のみ
...
[impl-manager]
  ...
  Phase 1: Investigation ─────────────────────────────────────────────
    → refs/<リポ名>/knowledge.md の既存知見を参照（存在する場合のみ）
    → phase-1-investigation/01_tasks.md 作成（既存知見がある場合は含める）
    ...
    → phase-1-investigation/06_gate.md にゲート判定
                                                                    |
  Phase 2: Design ────────────────────────────────────────────────────
    → refs/<リポ名>/knowledge.md の設計関連知見を参照（存在する場合のみ）
  ...
  Phase 3: Implementation Planning ───────────────────────────────────
    → refs/<リポ名>/knowledge.md の実装関連知見を参照（存在する場合のみ）
  ...
  Phase 4: Implementation（並列ディスパッチ: Wave 方式 + worktree）─────
    → refs/<リポ名>/knowledge.md から各タスク関連知見を抽出・要約（存在する場合のみ）
    ...
    → 事前準備:
      ...
      ・事前検証チェックリストを実行
    → Wave 1: 依存関係なしの IMPL-XXX を並列ディスパッチ（refs 知見要約を各 worker に渡す）
...
> 施策横断で再発しうる課題を `issues/entries/ISS-XXX.md` として作成します（CSV は自動生成）。
...
1. 事前調査      → sessions/metacognition/YYYYMMDD/00_pre_investigation.md を埋める
                   ...
                   ・プロセス改善_課題管理.csv（.md から自動生成）の現状把握
...
8. アクション実施 → 分析結果に基づくアクションを実施
                   ...
                   ・明確な課題 → issues/entries/ISS-XXX.md を作成（CSV は自動生成）
...
| MC-003 | 課題パターン分析 | `プロセス改善_課題管理.csv`（.md から自動生成） | 再発カテゴリ・停滞課題の検出 |
...
1. 事前調査      → sessions/backlog-maintenance/YYYYMMDD/00_pre_investigation.md を埋める
                   ...
                   ・backlog/backlog.csv（.md から自動生成）とファイル実体の整合性確認
                   ・sessions/initiatives/ および _archive/ の施策状況確認
                   ・プロセス改善_課題管理.csv（.md から自動生成）の関連課題確認
...
Phase 1: backlog-candidate-propose.yml (cron: 3時間ごと / workflow_dispatch)
  ...
  2. backlog/entries/ をパースし、対象条件に合致する最古の1件を選択
...
## 非起動型スキル一覧（user-invocable: false）

`user-invocable: false` のスキルは `/` メニューには表示されず、他のスキルやエージェントからオンデマンドで参照される。

### ポリシー系

| スキル | 説明 |
|--------|------|
| `manager-common-policy` | マネージャーセッション（l1, triage, metacognition, sync, backlog-maintenance, automation, config-optimizer, session-consistency, l1-impl）に共通する運用パターンを定義するポリシー |
| `session-flow-policy` | 新セッションタイプの作成・`.claude/skills/` 配下のスキル/エージェント構成の変更時に参照するポリシー |
| `session-lifecycle-policy` | セッションタイプ横断のライフサイクルパターン・ペアリング要件を定義するガイドライン |
| `manager-overview` | マネージャーセッション全体の責務・連携関係のオーバービュー。整合性管理や新規マネージャー設計時に参照 |

### チェックリスト系

| スキル | 説明 |
|--------|------|
| `rule-change-checklist` | ルール変更を含む施策で、連動更新の漏れを防ぐための標準チェックリスト |
| `format-conversion-checklist` | フォーマット変換（内容・ロジック変更なし）を含むタスクで、実用的情報の意図しない損失を防ぐための同等性検証チェックリスト |

### リファレンス系

| スキル | 説明 |
|--------|------|
| `builtin-skills-reference` | Claude Code ビルトイン skills の評価結果・適用可能性の参照 |
| `plugin-reference` | Claude Code 公式プラグインの評価結果・適用可能性の参照 |
| `gha-guideline` | GHA × Claude Code ワークフローの権限設計・変換パターンリファレンス |

### ユーティリティ系

| スキル | 説明 |
|--------|------|
| `external-repo-cleanup` | 外部リポジトリのブランチ整理をオンデマンドで実行。L1マネージャーが壁打ちフェーズ後に呼び出す |

### GHA 専用（上記「GHA Initiative スキル」セクション参照）

| スキル | 説明 |
|--------|------|
| `gha-wallbash` | GHA initiative-wallbash ワークフロー専用。壁打ちフェーズ（init/continue） |
| `gha-execute` | GHA initiative-execute ワークフロー専用。実行フェーズ（approve/reject） |
| `gha-close` | GHA initiative-close ワークフロー専用。クローズフェーズ |
| `gha-question` | GHA initiative-question ワークフロー専用。質問回答（読み取り専用） |
| `gha-test` | GHA 壁打ち相当の動作検証用テストスキル |
...
8. アクション実施 → レポートに基づくアクションを実施
                   ...
                   ・課題転記 → issues/entries/ISS-XXX.md を作成（CSV は自動生成）
9. PR 作成      → 全タスク完了後にコミット・プッシュし、PR を作成
10. ユーザーレビュー → PR でまとめてレビュー・承認
```
...
[session-consistency-manager]
  ...
  事前調査: sessions/session-consistency/YYYYMMDD/00_pre_investigation.md
    ...
    - session-lifecycle-policy / session-flow-policy / manager-common-policy の現状確認
    - 過去セッション確認
                                                                    |
  計画: 01_plan.md
...
> **正の情報源**: `.claude/skills/design-partner/SKILL.md`
> **特徴**: 単体スキル（マネージャー＋ワーカー構成ではない）。人間との直接対話で完結する。

```
[design-partner] ← ユーザーが /design-partner で起動
                                                                    |
  テーマ名決定 → sessions/design/<テーマ名>/ を作成
                                                                    |
  EXPLORE（発散・探索）
    → 目的発散（質問群をまとめて提示）
    → 制約明確化（チェックリスト形式）
    → アイデア収束（MoSCoW 分類）
    → brainstorm_log.md に記録
    → ユーザー承認
                                                                    |
  DEFINE（要件定義）
    → WHY/WHAT/HOW 分解（仮説提示型）
    → スコープ境界の確定（選択肢提示型）
    → 成功基準（KPI）の設定
    → design.md 前半に記録
    → ユーザー承認
  ...
  REVIEW（最終確認・成果物生成）
    → 全体サマリーの提示
    → backlog/entries/<施策名>.md を生成（設計成果物へのポインター付き）
    → 完了報告
```
...
| ファイル | フェーズ | 内容 |
|---------|---------|------|
| `sessions/design/<テーマ名>/brainstorm_log.md` | EXPLORE | 壁打ちログ |
| `sessions/design/<テーマ名>/design.md` | DEFINE + DESIGN | 要件定義 + 設計 |
| `sessions/design/<テーマ名>/action_plan.md` | PLAN + REVIEW | 実施計画 |
| `backlog/entries/<施策名>.md` | REVIEW | backlog エントリ（設計成果物へのポインター付き） |

### 既存フローとの関係

- design-partner は L1 マネージャーの **上流** に位置する
- design-partner の成果物（backlog エントリ）を L1 マネージャーが `backlog: <施策名>` で施策化する
- design-partner は `sessions/initiatives/` への直接書き込みは行わない

---

## プロジェクト進捗反映スキル

> **正の情報源**: `.claude/skills/project-progress-sync/SKILL.md`
> **特徴**: 単体スキル（ユーザー起動）。`projects/<name>/` のプロジェクト管理ファイルを更新する。

```
[project-progress-sync] <- ユーザーが /project-progress-sync <project-name> で起動
                                                                    |
  projects/<name>/todo.md を読み込み、施策名一覧を取得
                                                                    |
  sessions/initiatives/ + _archive/ を走査し、各施策の状態を判定
    → _archive/ 配下 → 完了
    → 05_gate_review.md 存在（通過）→ 完了
    → 05_gate_review.md 存在（通過以外）→ 進行中（評価完了）
    → phase-2-execution/ 存在 → 進行中
    → 01_proposal.md のみ → 未着手（提案のみ）
    → ディレクトリなし → 未着手
                                                                    |
  projects/<name>/todo.md のステータスを更新
                                                                    |
  完了施策のレポートから知見を抽出
    → phase-2-execution/set-N/03_worker_report.md の「作業中の知見」
    → phase-2-execution/set-N/06_eval_report.md の「評価中の知見」
                                                                    |
  projects/<name>/knowledge.md に知見を追記
                                                                    |
  計画修正が必要な場合 → projects/<name>/plan.md を更新
    （施策ブロック・スコープ変更・新規依存関係の場合のみ）
                                                                    |
  結果報告（ステータス変更・知見件数・plan 変更内容）
```

### 関連スキル

- **project-init**: プロジェクト初期化（`projects/<name>/` ディレクトリ作成）。project-progress-sync の前工程にあたる
- **l1-manager**: 施策の計画・実行・ゲート判定。project-progress-sync は施策の状態を読み取るのみで、施策ファイルの変更は行わない

---

## スプリントセッション フロー

> **正の情報源**: `.claude/skills/sprint-planner/SKILL.md`, `.claude/skills/sprint-generator/SKILL.md`, `.claude/skills/sprint-evaluator/SKILL.md`
> **特徴**: 3つの独立した skill（planner / generator / evaluator）を人間がオーケストレートする。マネージャー＋ワーカー構成ではなく、対等な3ロール構成。

```
[sprint-planner] ← ユーザーが /sprint-planner で起動
  パターン1: sprint: new <sprint_name>（新規）
    → テンプレートコピー（sessions/sprints/_template/ → sessions/sprints/<sprint_name>/）
    → 壁打ち（目標・スコープ・成功条件・タスク詳細）
    → plan.md 記入
    → todo.md 記入（親タスク T-XXX + 子タスク T-XXX-Y）
    → 次のアクションをガイド（→ /sprint-generator）

  パターン2: sprint: <sprint_name>（再開）
    → フォルダ状態確認 → 分岐:
      - todo.md が全 todo → generator へのガイド
      - DEVIATION あり → plan/todo を修正
      - 未対応 IMP あり → plan に対応方針追記・todo 更新
      - 全タスク done → 親タスク done 更新 → knowledge 蒸留 → _archive 移動

[sprint-generator] ← ユーザーが /sprint-generator で起動
  → plan.md・todo.md を読む
  → 着手可能タスクを自律判断して一括提示
  → ユーザー承認後に自走
  → T-XXX-Y を実行、work_log.md に記録
  → todo.md のステータス更新（子タスクのみ）
  → 次のアクションをガイド（→ /sprint-evaluator or /sprint-planner）

[sprint-evaluator] ← ユーザーが /sprint-evaluator で起動
  → plan.md の成功条件を読む
  → todo.md で done タスクを確認
  → work_log.md を読む（作業経緯の把握）
  → Round N として review.md に追記（上書き禁止）
  → 成功条件チェック（PASS / FAIL / PARTIAL）
  → 指摘事項（IMP-XXX）を記録
  → 次のアクションをガイド（→ /sprint-planner）
```
...
| ファイル | 管理ロール | 内容 |
|---------|----------|------|
| `sessions/sprints/<sprint_name>/plan.md` | planner | スプリント計画 |
| `sessions/sprints/<sprint_name>/todo.md` | planner（作成）/ generator（ステータス更新） | タスクリスト |
| `sessions/sprints/<sprint_name>/work_log.md` | generator | 作業ログ |
| `sessions/sprints/<sprint_name>/review.md` | evaluator | レビュー記録 |
| `sessions/sprints/<sprint_name>/knowledge.md` | planner | 知見蒸留 |

### 既存フローとの関係

- スプリントセッションは L1/L2 イニシアティブフローとは**独立**した並列のワークフロー
- イニシアティブは L1 マネージャーがオーケストレーターだが、スプリントは**人間**がオーケストレーター
- `sessions/sprints/` 配下で完結し、`sessions/initiatives/` とは干渉しない
...
session-lifecycle-policy   ← 「ライフサイクル・ペアリング」（横断的整合性基準）
...
| §10 | ドメインエージェント参照 | refs/agency-agents/knowledge.md のカタログから施策タイプに適したエージェントを選定し、L2 に専門知識として参照させる（任意）。§10.4 で Evaluator 向け具体的手順（評価計画への反映・評価基準への活用・有効性フィードバック）を定義 |
...
3. **TG-008 基準の連動更新**: `session-lifecycle-policy/SKILL.md` のチェック基準に影響する場合は更新する
...
| 管理単位 | 役割 | 格納先 |
|---------|------|--------|
| CSV（`プロセス改善_課題管理.csv`） | 全体俯瞰のハブ（.md から自動生成） | リポジトリルート |
...
2. 対応するかどうかを判断し、対応・クローズ情報セクションの `対応方針（確定）` と `確定日` を記入する
3. タスク化する場合は基本情報の `ステータス` を `タスク化済` に、`タスク化先` を記入する
4. 対応しない場合は `ステータス` を `対応不要` にして `備考` に理由を書く
