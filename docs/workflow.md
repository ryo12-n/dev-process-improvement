...
[L1 マネージャー]
  ...
  Phase 1: 調査・方針決定・計画作成
    ...
    → Phase 1→2 チェック記録（05_gate_review.md — Plan-Worker ディスパッチ前チェック3項目 + フェーズコスト小計）
...
[L2 計画ワーカー（l2-plan-worker）] ← L1 が Task ツールでサブエージェントとして起動
  ...
  L1: plan-worker 成果物を確認（スコープ網羅・マトリクス完全性・循環依存なし）
    ...
    + Phase 2→3 チェック記録（05_gate_review.md — Plan-Evaluator ディスパッチ前チェック4項目 + フェーズコスト小計）
    → 品質不十分なら差し戻し
                                                                    ↓
  Phase 3: 計画評価・タスク作成
[L2 計画評価者（l2-plan-evaluator）] ← L1 が Task ツールでサブエージェントとして起動
    壁打ちフェーズ → phase-1-planning/set-N/04_eval_plan.md → コンフリクトマトリクス・依存グラフ・Wave割当の正確性を評価
    → phase-1-planning/set-N/06_eval_report.md を作成
                                                                    ↓
  L1: 04_dispatch_log.md に Plan-Evaluator 完了エントリ（コストデータ含む）を記録
    + Phase 3→4 チェック記録（05_gate_review.md — Worker ディスパッチ前チェック5項目 + フェーズコスト小計）
...
[L2 ワーカー（実施）] ← L1 が Task ツールでサブエージェントとして起動
  ...
  L1: worker 成果物を確認（タスク完了状態・課題転記状態）
    ...
    + Phase 4→5 チェック記録（05_gate_review.md — Evaluator ディスパッチ前チェック8項目 + フェーズコスト小計を記録）
    + 06_cost_record.md にフェーズコスト小計を記録
    → 品質不十分なら差し戻し
                                                                    ↓
...
### フェーズゲートチェック（05_gate_review.md への累積記録）

L1 マネージャーは施策のライフサイクル中、5段階のフェーズチェックを `05_gate_review.md` に累積記録する。
...
各チェックの結果は同一ファイル（`05_gate_review.md`）に順次追記され、施策全体のフェーズ遷移記録として残る。
...
2. Phase 3→4: 03_tasks.md 作成（L1がphase-1-planning/set-N/08_task_division.mdを参考に作成）
