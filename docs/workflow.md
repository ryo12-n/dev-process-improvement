...
[impl-manager]
  ...
  Phase 3: Implementation Planning ───────────────────────────────────
    ...
    → 35_impl_plan_gate.md にゲート判定
                                                                    |
  Phase 4: Implementation（並列ディスパッチ: Wave 方式）──────────────
    → 40_impl_tasks.md を 36 から生成
    → Wave 1: 依存関係なしの IMPL-XXX を並列ディスパッチ
      各 impl-worker: コード実装 → テスト（max 3 リトライ）
                      → チェックポイント記録（41_impl_work_log_W<N>.md）
                      → COMPLETE or ESCALATE
    → [Wave N: 依存解消後に追加ディスパッチ]
    → B+C 集約: git log + 全 work_log から 42_impl_work_report.md 構築
...
- **B (マネージャー集約)**: `git -C <external-repo> log --oneline initiative/<branch>` + 全 `41_impl_work_log_W*.md` を読み、`42_impl_work_report.md` を構築
