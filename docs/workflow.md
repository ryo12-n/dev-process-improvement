...
[L1 マネージャー]
  ...
  Phase 1: 調査・方針決定・計画作成
    ...
    → Phase 1→2 チェック記録（08_gate_review.md — Plan-Worker ディスパッチ前チェック3項目）
                                                                    ↓
...
[L2 計画ワーカー（l2-plan-worker）] ← L1 が Task ツールでサブエージェントとして起動
  ...
  L1: plan-worker 成果物を確認（スコープ網羅・マトリクス完全性・循環依存なし）
    + Phase 2→3 チェック記録（08_gate_review.md — Plan-Evaluator ディスパッチ前チェック4項目）
    → 品質不十分なら差し戻し
                                                                    ↓
  Phase 3: 計画評価・タスク作成
[L2 計画評価者（l2-plan-evaluator）] ← L1 が Task ツールでサブエージェントとして起動
    ...
    → 06_eval_report.md を作成
                                                                    ↓
  L1: Phase 3→4 チェック記録（08_gate_review.md — Worker ディスパッチ前チェック5項目）
...
[L2 ワーカー（実施）] ← L1 が Task ツールでサブエージェントとして起動
  ...
  L1: worker 成果物を確認（タスク完了状態・課題転記状態）
    + Phase 4→5 チェック記録（08_gate_review.md — Evaluator ディスパッチ前チェック8項目を記録）
    → 品質不十分なら差し戻し
                                                                    ↓
...
[L2 ワーカー（評価）] ← L1 が Task ツールでサブエージェントとして起動（worker 完了後に順次起動）
  ...
  Phase 6: レビュー・ゲート判定
[L1 マネージャー]
  Phase 5→6 チェック記録（08_gate_review.md — 最終ゲート判定前チェック5項目を記録）
...
| チェック | タイミング | 項目数 | 内容 |
|---------|-----------|--------|------|
| Phase 1→2 | Plan-Worker ディスパッチ前 | 3項目 | 提案の完全性・成功基準の評価可能性・施策ブランチ |
| Phase 2→3 | Plan-Evaluator ディスパッチ前 | 4項目 | スコープ網羅・マトリクス完全性・循環依存なし・計画レポート |
| Phase 3→4 | Worker ディスパッチ前 | 5項目 | 計画評価完了・タスクID/完了条件/優先度・固定タスク・Set Assignment・ドメインエージェント分離 |
| Phase 4→5 | Evaluator ディスパッチ前 | 8項目 | タスク分類・レポート記載・計画対比実績・知見セクション・課題バッファ・CSV転記・外部リポ・メタルール検証 |
| Phase 5→6 | 最終ゲート判定前 | 5項目 | 評価計画カバー率・エビデンス品質・知見セクション・未転記課題・方針推奨 |
...
[session-consistency-manager]
  ...
  計画: 01_plan.md
    ...
    - フェーズ計画作成
                                                                    |
  Phase 1: 情報収集 ─────────────────────────────────────────────────
    タスク作成: phase-1-collection/workers/set-1/01_tasks.md         |
    ワーカー: sc-collection-worker                                   |
      → reference/session-consistency-reference.md を更新             |
      → phase-1-collection/workers/set-1/04_scan_report.md を作成    |
    評価者: sc-evaluator（Phase 1 コンテキスト）                      |
    ゲート: phase-1-collection/01_gate.md                             |
                                                                    |
  Phase 2: 現状分析 ─────────────────────────────────────────────────
    タスク作成: phase-2-analysis/workers/set-1/01_tasks.md           |
    ワーカー: sc-analysis-worker                                     |
      → セッション定義をリファレンスと比較分析                        |
      → phase-2-analysis/workers/set-1/04_scan_report.md を作成      |
    評価者: sc-evaluator（Phase 2 コンテキスト）                      |
    ゲート: phase-2-analysis/01_gate.md                               |
                                                                    |
  Phase 3: 改善提案 ───────────────────────────────────────────────
    タスク作成: phase-3-proposal/workers/set-1/01_tasks.md           |
    ワーカー: sc-proposal-worker                                     |
      → Impact × Effort 優先順位付き提案を作成                      |
      → phase-3-proposal/workers/set-1/04_scan_report.md を作成      |
