...
起動時に渡す情報: 役割とルールファイル、施策パス、タスク範囲、完了の定義、参照ドメインエージェント（Worker/Evaluator 別に指定可。任意、§10.3 + §10.5 参照）
...
| §10 | ドメインエージェント参照 | refs/agency-agents/knowledge.md のカタログから施策タイプに適したエージェントを選定し、L2 に専門知識として参照させる（任意）。§10.5 で Evaluator 向け具体的手順（評価計画への反映・評価基準への活用・有効性フィードバック）を定義 |
