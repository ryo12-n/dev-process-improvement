...
3フェーズ構成セッション定義のメタテンプレート（ジェネレータ）が存在しない。新しいセッション種別を追加する際に既存セッション定義を手動で参照・コピーしており、構造の一貫性を保つのが困難。

## 期待効果

新規セッション種別の追加が容易になり、セッション定義の構造的一貫性が向上する。
