...
`01_plan.md` の成功基準で `grep -r` を使う場合、対象ディレクトリが施策スコープと一致しないと評価時に false positive が発生する。成功基準に grep を含める際の対象パス制限に関するガイドラインが存在しないため、施策スコープ外の一致が成功判定に混入するリスクがある。

## 期待効果

- 成功基準に grep を含める場合に対象パスを施策スコープに限定する記述を推奨するガイドラインを追加することで、評価時の false positive を防止できる
- L2-evaluator の評価精度が向上する

## 補足・参考情報

- 発見元: 施策 phase-worker-set-standardization（L2-evaluator E-1）
