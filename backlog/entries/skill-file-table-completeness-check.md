...
スキルファイル修正時の担当ファイルテーブル網羅性チェックがタスク完了条件に含まれていない。スキルファイルの変更によって担当ファイルテーブルとの不整合が生じるリスクがある。

## 期待効果

スキルファイル変更時に担当ファイルテーブルの網羅性が自動的にチェックされ、不整合の見落としが防止される。
