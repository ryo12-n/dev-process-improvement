# GHA環境でのgit cloneによる外部リポジトリアクセスパターン
...
GHA環境でのgit cloneによる外部リポジトリアクセスパターンのドキュメント化が必要。GHA環境では1リポジトリ制約があるが、git cloneを用いた外部リポジトリへのアクセスパターンが標準化されていない。

## 期待効果

GHA環境での外部リポジトリアクセスパターンが標準化され、横断施策のGHA自動化が容易になる。
