...
| `refs/` | 他PJの実装・構成など、施策の参考にする参照物置き場。inbox にポインターメモを置き、本体はここに置く。外部リポジトリ固有の知見は `<リポ名>/knowledge.md` に蓄積する（`manager-common-policy` §5.3 参照）。調査過程の生データは `<リポ名>/work-logs/` に記録する。**参照物格納時は用途・有効期限のメタ情報を必ず記載すること** |
