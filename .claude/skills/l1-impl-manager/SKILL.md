...
4. Phase 1: Investigation
   - 4a. investigation tasks 作成（phase-1-investigation/01_tasks.md）
   - 4b. investigation-worker ディスパッチ
   - 4c. investigation 成果物確認
   - 4d. impl-evaluator ディスパッチ（Phase 1）
   - 4e. investigation gate 判定（phase-1-investigation/06_gate.md）
5. Phase 2: Design
   - 5a. design tasks 作成（phase-2-design/01_tasks.md）
   - 5b. design-worker ディスパッチ
   - 5c. design 成果物確認
   - 5d. impl-evaluator ディスパッチ（Phase 2）
   - 5e. design gate 判定（phase-2-design/06_gate.md）
6. Phase 3: Implementation Planning
   - 6a. impl-plan tasks 作成（phase-3-impl-plan/01_tasks.md）
   - 6b. impl-plan-worker ディスパッチ
   - 6c. impl-plan 成果物確認（phase-3-impl-plan/07_file_task_division.md の品質重点チェック）
   - 6d. impl-evaluator ディスパッチ（Phase 3）
   - 6e. impl-plan gate 判定（phase-3-impl-plan/06_gate.md）
7. Phase 4: Implementation
   - 7a. impl tasks 作成（phase-4-impl/01_tasks.md, from phase-3-impl-plan/07_file_task_division.md）
   - 7b. impl-worker 並列ディスパッチ（Wave 1）
   - 7c. [Wave N if needed]
   - 7d. work report 構築（phase-4-impl/03_work_report.md, B+C aggregation）
   - 7e. impl-evaluator ディスパッチ（Phase 4）
   - 7f. impl gate 判定（phase-4-impl/06_gate.md）
...
1. **refs 知見の参照**: 外部リポジトリがある場合、`refs/<リポジトリ名>/knowledge.md` が存在するか確認する。存在する場合は読み込み、過去の調査で蓄積された知見（リポジトリ構造、既知の制約、ハマりどころ等）を把握する。investigation-worker の調査タスクに既存知見を含め、重複調査を避ける指示を追加する
   - **注意**: 知見は参考情報として扱い、最新コードベースの調査を省略しない旨をタスクに明記すること
   - knowledge.md が存在しない場合はこのステップをスキップする
2. **タスク作成**: `phase-1-investigation/01_tasks.md` にコードベース調査タスクを記載
3. **ワーカーディスパッチ**: investigation-worker を起動（成果物: phase-1-investigation/02, 03, 07）
4. **成果物確認**: §3 + 調査対象の網羅性チェック
5. **評価者ディスパッチ**: impl-evaluator を Phase 1 コンテキストで起動（成果物: phase-1-investigation/04, 05）
6. **ゲート判定**: `phase-1-investigation/06_gate.md` に判定（調査が十分か、設計フェーズに進めるか）
7. **refs 概要の自動抽出**: 外部リポジトリがある場合、`phase-1-investigation/03_work_report.md` のリポジトリ概要を `refs/<リポジトリ名>/knowledge.md` に抽出する（`manager-common-policy` §5.3 の手順に従う）

### Phase 2: Design

1. **refs 知見の参照**: `refs/<リポジトリ名>/knowledge.md` が存在する場合は読み込み、設計に関連する知見（アーキテクチャ制約、過去の設計判断、既知の技術的制約等）を確認する。design-worker への指示に関連知見を含める
   - knowledge.md が存在しない場合はこのステップをスキップする
2. **タスク作成**: `phase-2-design/01_tasks.md` に設計タスクを記載
3. **ワーカーディスパッチ**: design-worker を起動（成果物: 02_impl_design, phase-2-design/02, 03, 07）
4. **成果物確認**: §3 + `02_impl_design.md` のコンポーネント設計・API変更・テスト方針の記載チェック
5. **評価者ディスパッチ**: impl-evaluator を Phase 2 コンテキストで起動（成果物: phase-2-design/04, 05）
6. **ゲート判定**: `phase-2-design/06_gate.md` に判定（設計が実装計画に進められるか）

### Phase 3: Implementation Planning

1. **refs 知見の参照**: `refs/<リポジトリ名>/knowledge.md` が存在する場合は読み込み、実装計画に関連する知見（複雑度の高い箇所、既知のハマりどころ、過去のタスク分割パターン等）を確認する。impl-plan-worker への指示に関連知見を含める
   - knowledge.md が存在しない場合はこのステップをスキップする
2. **タスク作成**: `phase-3-impl-plan/01_tasks.md` に実装計画タスクを記載
3. **ワーカーディスパッチ**: impl-plan-worker を起動（成果物: phase-3-impl-plan/07, 02, 03, 07）
4. **成果物確認**: §3 + **`phase-3-impl-plan/07_file_task_division.md` の品質重点チェック**:
   - IMPL-XXX 形式のタスク分割が適切か
   - Conflict Check Matrix に矛盾がないか（同一ファイルを複数タスクが MODIFY していないか）
   - テストコマンドが全タスクに指定されているか
   - 依存関係グラフが整合しているか
5. **評価者ディスパッチ**: impl-evaluator を Phase 3 コンテキストで起動（成果物: phase-3-impl-plan/04, 05）
6. **ゲート判定**: `phase-3-impl-plan/06_gate.md` に判定（並列実装に進められるか）

### Phase 4: Implementation

1. **refs 知見の抽出・要約**: `refs/<リポジトリ名>/knowledge.md` が存在する場合は読み込み、各 IMPL-XXX タスクに関連する知見（該当ファイルの既知の制約、過去のハマりどころ等）を抽出・要約する。この要約は Wave ディスパッチ時に各 impl-worker に渡す（impl-worker 自身は refs を直接読まない設計）
   - knowledge.md が存在しない場合はこのステップをスキップする
2. **タスク作成**: `phase-4-impl/01_tasks.md` を `phase-3-impl-plan/07_file_task_division.md` から生成
3. **並列ディスパッチ**: 下記「並列ディスパッチ機構」に従う
4. **Work Report 構築**: 下記「B+C 作業ログ集約手順」に従い `phase-4-impl/03_work_report.md` を構築
5. **評価者ディスパッチ**: impl-evaluator を Phase 4 コンテキストで起動（成果物: phase-4-impl/04, 05）
6. **ゲート判定**: `phase-4-impl/06_gate.md` に判定（実装が完了しているか、テストが通るか）
...
3. per-worker ワークログファイルを作成:
   `phase-4-impl/02_work_log_W<N>.md` を `phase-4-impl/02_work_log_W_template.md` からコピーし、以下の既知値を事前記入する:
   - **Worker ID**: W<N>
   - **担当タスク**: IMPL-XXX（`phase-3-impl-plan/07_file_task_division.md` から取得）
   - **Wave**: Wave M
   - **Worktree パス**: `<ext-repo>/.worktrees/impl-W<N>`
   - **ブランチ**: `impl/<施策名>-W<N>`
   - **壁打ちフェーズの「対象ファイル」「テストコマンド」**: `phase-3-impl-plan/07_file_task_division.md` から取得
...
4. [ ] per-worker ワークログファイルが作成済み
   検証: ls sessions/impl/<施策名>/phase-4-impl/02_work_log_W*.md
...
1. `phase-3-impl-plan/07_file_task_division.md` を解析し、依存関係なしのタスクを Wave 1 として選定
2. 各 impl-worker に以下を渡してサブエージェントとして起動:
   - IMPL-XXX タスク番号と内容
   - 設計ドキュメントパス（`02_impl_design.md`）
   - 外部リポジトリのローカルパス
   - 施策ブランチ名
   - ワークログパス（`phase-4-impl/02_work_log_W<N>.md`）
   - Worker ID（`W<N>`）
   - 外部リポ worktree パス（`<ext-repo>/.worktrees/impl-W<N>`）
   - per-worker ブランチ名（`impl/<施策名>-W<N>`）
   - 課題ファイルパス（`07_issues_W<N>.md`）
   - **refs 知見要約**（存在する場合）: 当該タスクの担当ファイルに関連する既存知見の要約。impl-worker は refs を直接読まず、この要約を参考情報として活用する
...
- `phase-3-impl-plan/07_file_task_division.md` の Conflict Check Matrix で、同一 Wave 内のタスク間にファイル競合がないことを確認する
...
5. worktree をクリーンアップ:
   ...
   ```
6. `phase-4-impl/01_tasks.md` のステータス更新

### 事後検証チェックリスト

```
1. [ ] 全ワーカーが COMPLETE or ESCALATE を記録済み
   検証: 各 phase-4-impl/02_work_log_W<N>.md の最終チェックポイント確認
   回復:
     ...
     c. phase-4-impl/03_work_report.md の「Reconstruction Count」に復元件数を記録
2. [ ] per-worker 課題ファイルが存在（空でも可）
   検証: 各 07_issues_W<N>.md の存在・内容確認
   回復: 未作成 → phase-4-impl/03_work_report.md に「課題ファイル未作成」を記録（非ブロッキング）
...
5. [ ] 変更ファイルが Assigned files の範囲内
   検証: git -C <ext-repo> diff --name-only impl/<施策名>..impl/<施策名>-W<N>
          を phase-3-impl-plan/07_file_task_division.md と照合
...
5. 解消結果を `phase-4-impl/03_work_report.md` の「Merge Results」に記録

---

## B+C 作業ログ集約手順

Phase 4 完了後、マネージャーが `phase-4-impl/03_work_report.md` を構築する。
...
- 全 `phase-4-impl/02_work_log_W*.md` を読み、ワーカーのチェックポイント記録を集約する
- 全 `07_issues_W*.md` を読み、ワーカー別の課題を `07_issues.md` に統合する

### C: ワーカーによるチェックポイント記録

各 impl-worker が `phase-4-impl/02_work_log_W<N>.md` に以下のチェックポイントを記録:
...
Phase 1 タスク作成時に、`manager-common-policy` §10 に従いドメインエージェントの選定を行う。

1. `refs/agency-agents/knowledge.md` の「施策タイプ別推奨マッピング」を参照する
2. 施策タイプに合致するエージェントがあれば、そのエージェントの `.md` ファイルを Read して関連性を確認する
3. `phase-1-investigation/01_tasks.md` の「参照ドメインエージェント」セクションに選定結果を記載する（該当なしの場合は「なし」）

**判断基準**: 施策の専門性がコード実装の一般知識を超える場合（例: セキュリティ設計、GHA パイプライン構築、テスト戦略設計、ワークフロー設計）に参照を推奨する。単純なコード変更（関数追加、設定変更等）では省略してよい。
...
| `01_plan.md` | 作成・編集 |
| `phase-1-investigation/01_tasks.md` | 作成・編集 |
| `phase-1-investigation/06_gate.md` | 作成・編集 |
| `phase-2-design/01_tasks.md` | 作成・編集 |
| `phase-2-design/06_gate.md` | 作成・編集 |
| `phase-3-impl-plan/01_tasks.md` | 作成・編集 |
| `phase-3-impl-plan/06_gate.md` | 作成・編集 |
| `phase-4-impl/01_tasks.md` | 作成・編集 |
| `phase-4-impl/03_work_report.md` | 作成・編集（B+C 集約） |
| `phase-4-impl/06_gate.md` | 作成・編集 |
...
- B+C 方式で `phase-4-impl/03_work_report.md` を構築する
- 各フェーズのゲート判定を行う
...
impl 固有の入力ソース:
- `phase-1-investigation/03_work_report.md` の知見
