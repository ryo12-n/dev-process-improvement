...
1. **タスク作成**: `phase-1-investigation/01_tasks.md` にコードベース調査タスクを記載
2. **ワーカーディスパッチ**: investigation-worker を起動（成果物: phase-1-investigation/02, 03, 07）
3. **成果物確認**: §3 + 調査対象の網羅性チェック
4. **評価者ディスパッチ**: impl-evaluator を Phase 1 コンテキストで起動（成果物: phase-1-investigation/04, 05）
5. **ゲート判定**: `phase-1-investigation/06_gate.md` に判定（調査が十分か、設計フェーズに進めるか）
6. **refs 概要の自動抽出**: 外部リポジトリがある場合、`phase-1-investigation/03_work_report.md` のリポジトリ概要を `refs/<リポジトリ名>/knowledge.md` に抽出する（`manager-common-policy` §5.3 の手順に従う）

### Phase 2: Design

1. **タスク作成**: `phase-2-design/01_tasks.md` に設計タスクを記載
2. **ワーカーディスパッチ**: design-worker を起動（成果物: 02_impl_design, phase-2-design/02, 03, 07）
3. **成果物確認**: §3 + `02_impl_design.md` のコンポーネント設計・API変更・テスト方針の記載チェック
4. **評価者ディスパッチ**: impl-evaluator を Phase 2 コンテキストで起動（成果物: phase-2-design/04, 05）
5. **ゲート判定**: `phase-2-design/06_gate.md` に判定（設計が実装計画に進められるか）

### Phase 3: Implementation Planning

1. **タスク作成**: `phase-3-impl-plan/01_tasks.md` に実装計画タスクを記載
2. **ワーカーディスパッチ**: impl-plan-worker を起動（成果物: phase-3-impl-plan/07, 02, 03, 07）
3. **成果物確認**: §3 + **`phase-3-impl-plan/07_file_task_division.md` の品質重点チェック**:
   - IMPL-XXX 形式のタスク分割が適切か
   - Conflict Check Matrix に矛盾がないか（同一ファイルを複数タスクが MODIFY していないか）
   - テストコマンドが全タスクに指定されているか
   - 依存関係グラフが整合しているか
4. **評価者ディスパッチ**: impl-evaluator を Phase 3 コンテキストで起動（成果物: phase-3-impl-plan/04, 05）
5. **ゲート判定**: `phase-3-impl-plan/06_gate.md` に判定（並列実装に進められるか）

### Phase 4: Implementation

1. **タスク作成**: `phase-4-impl/01_tasks.md` を `phase-3-impl-plan/07_file_task_division.md` から生成
2. **並列ディスパッチ**: 下記「並列ディスパッチ機構」に従う
3. **Work Report 構築**: 下記「B+C 作業ログ集約手順」に従い `phase-4-impl/03_work_report.md` を構築
4. **評価者ディスパッチ**: impl-evaluator を Phase 4 コンテキストで起動（成果物: phase-4-impl/04, 05）
5. **ゲート判定**: `phase-4-impl/06_gate.md` に判定（実装が完了しているか、テストが通るか）
