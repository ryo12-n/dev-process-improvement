...
1. セッションフォルダを作成する: `sessions/backlog-maintenance/_template/` 直下のファイル（00〜04）のみを `sessions/backlog-maintenance/YYYYMMDD/` にコピーし、`sessions/backlog-maintenance/YYYYMMDD/phase-1-scan/` は空ディレクトリとして作成する（`_template/phase-1-scan/_template/` はコピーしない）
2. `00_pre_investigation.md` の穴埋めを実施する（現状把握）
   - `backlog/entries/` 内の全エントリの起票日・優先度・ステータスを確認する
   - `backlog/backlog.csv`（.md から自動生成）と `backlog/entries/` の整合性を確認する
...
1. `sessions/backlog-maintenance/_template/phase-1-scan/_template/` を `sessions/backlog-maintenance/YYYYMMDD/phase-1-scan/set-N/` にコピーする（セッションフォルダ内の `phase-1-scan/_template/` ではなく、リポジトリの `_template/phase-1-scan/_template/` から直接コピーする）
2. `phase-1-scan/set-N/01_tasks.md` に割り当てる BM タスクを記入する
...
1. 各 `phase-1-scan/set-N/04_scan_report.md` の走査結果を BM タスク別にマージする
...
| 明確な課題（対応が必要） | `issues/entries/ISS-XXX.md` を作成して転記（CSV は自動生成） | 「backlog 管理プロセスの改善が必要」 |
...
| `sessions/backlog-maintenance/YYYYMMDD/phase-1-scan/set-N/01_tasks.md` | 作成（ワーカーへのタスク指示） |
| `sessions/backlog-maintenance/YYYYMMDD/phase-1-scan/set-N/06_eval_report.md` | 読み取りのみ（知見集約の入力） |
| `sessions/backlog-maintenance/YYYYMMDD/phase-1-scan/set-N/07_issues.md` | 読み取り＋転記済みマーク追記（課題集約） |
...
| `sessions/backlog-maintenance/_template/phase-1-scan/_template/07_issues.md` | ワーカーセット別課題バッファのテンプレート |
