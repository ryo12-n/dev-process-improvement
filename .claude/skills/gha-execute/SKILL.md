...
9. ゲート判定サマリーを標準出力に出力する（Action が Issue コメントとして投稿する）
   - 出力の末尾に `GATE_VERDICT: 通過` または `GATE_VERDICT: 差し戻し` を独立行として含める
10. コミット・プッシュする
...
- ゲート判定サマリーの標準出力への出力
- ゲート判定サマリー出力時に `GATE_VERDICT: 通過` または `GATE_VERDICT: 差し戻し` マーカーを末尾の独立行として出力
- コミット・プッシュ
...
- ゲート判定サマリー出力の末尾には必ず `GATE_VERDICT: <判定>` を独立行として含める。`<判定>` は `通過` または `差し戻し` のいずれか
