...
GHA ワークフロー間で責務（ステップ・ジョブ）を移動する際に確認すべき3つの検証パターン。

| # | 検証パターン | 説明 | 確認方法 |
|---|------------|------|---------|
| 1 | 移動元ステップの残存確認 | 責務を移動した後、移動元ワークフローに当該ステップが残存していないか | 移動元 YAML を検索し、移動対象のステップ名・コマンドが残っていないことを確認 |
| 2 | 移動先の条件付きステップ確認 | 移動先ワークフローに `if:` ガードが正しく設定されているか | 移動先 YAML の該当ステップに適切な `if:` 条件（トリガー種別・入力値等）が設定されていることを確認 |
| 3 | 既存リソースの重複作成防止 | 移動先で既存リソース（ブランチ・PR・Issue 等）の重複作成を防ぐロジックがあるか | 冪等性チェック（既存の場合はスキップ or 更新）のロジックが実装されていることを確認 |

### 7.7 用途クラス別「やらないこと」テンプレート

セクション7.2 の共通4項目に加え、用途クラスごとに以下の差分を適用する。

#### 書き込み系（wallbash / execute / close）
...
ファイル書き込みと git/gh 操作を全面禁止する。

```markdown
## やらないこと
- ファイルの作成・編集（Write / Edit ツールの使用禁止）
- `git` コマンドの実行（読み取り専用モード）
- `gh` コマンドの実行（読み取り専用モード）
```

#### 検証用（test）

テスト目的に特化した独自構成。全権限の動作確認が目的のため、禁止事項は最小限。
