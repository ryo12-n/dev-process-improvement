...
## 8. スキル/プラグイン機能重複チェック（CO-008）

### チェック観点

| チェック対象 | 比較元 | 比較先 | 観点 |
|------------|--------|--------|------|
| スキル間重複 | `.claude/skills/*/SKILL.md` | 同左（全ペア比較） | description・機能カテゴリの類似度 |
| プラグイン-スキル重複 | `plugin-reference` SKILL.md の高・中適用性プラグイン | `.claude/skills/*/SKILL.md` | 機能の重複・代替可能性 |
| ビルトイン-カスタム重複 | `builtin-skills-reference` SKILL.md | `.claude/skills/*/SKILL.md` | ビルトインで代替可能なカスタムスキル |

### 重複の分類基準

| 分類 | 定義 | アクション |
|------|------|----------|
| 完全重複 | 同一の機能を提供し、一方が他方を完全に包含する | 統合・廃止を提案 |
| 部分重複 | 機能の一部が重複するが、それぞれ固有の機能も持つ | 責務分界の明確化を提案 |
| 補完関係 | 重複に見えるが実際には異なる文脈・用途で使い分けられる | 記録のみ（提案対象外） |

### 機能カテゴリの分類軸

- **チェック系**: 整合性確認、バリデーション、ドリフト検出
- **生成系**: ドキュメント生成、テンプレート適用
- **分析系**: 現状分析、パターン検出、統計
- **管理系**: オーケストレーション、ディスパッチ、ゲート判定
- **参照系**: リファレンス提供、評価結果の参照
