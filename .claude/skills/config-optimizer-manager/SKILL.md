...
1. セッションフォルダを作成する: `sessions/config-optimization/_template/` 直下のファイル（00〜04）のみを `sessions/config-optimization/YYYYMMDD/` にコピーし、各 `phase-N-xxx/` ディレクトリと `01_gate.md` を作成する（`phase-N-xxx/_template/` はコピーしない）
...
| CO-008 | スキル/プラグイン機能重複 | スキル間の機能重複、プラグインとスキルの重複、builtin-skills との重複 |
...
1. **セット作成**: `sessions/config-optimization/_template/phase-1-collection/_template/` を `phase-1-collection/set-1/` にコピーする
2. **タスク作成**: `phase-1-collection/set-1/01_tasks.md` に CO-001〜CO-007 の情報収集タスクを記載
...
1. **セット作成**: `sessions/config-optimization/_template/phase-2-analysis/_template/` を `phase-2-analysis/set-1/` にコピーする
2. **タスク作成**: `phase-2-analysis/set-1/01_tasks.md` に CO-001〜CO-007 の現状分析タスクを記載
...
1. **セット作成**: `sessions/config-optimization/_template/phase-3-proposal/_template/` を `phase-3-proposal/set-1/` にコピーする
2. **タスク作成**: `phase-3-proposal/set-1/01_tasks.md` に CO-001〜CO-007 の提案作成タスクを記載
...
1. `sessions/config-optimization/_template/phase-N-xxx/_template/` を `sessions/config-optimization/YYYYMMDD/phase-N-xxx/set-1/` にコピーする（該当フェーズの `_template/phase-N-xxx/_template/` から直接コピーする）
...
1. 各 `phase-N-xxx/set-1/04_scan_report.md` の分析結果をフェーズ別にマージする
2. 各 `phase-N-xxx/set-1/06_eval_report.md` の評価結果を「Phase サマリ」セクションに反映する
...
| 明確な課題（対応が必要） | `issues/entries/ISS-XXX.md` を作成して転記（CSV は自動生成） | 「path-scoped rules が未活用」 |
...
| `sessions/config-optimization/YYYYMMDD/phase-1-collection/01_gate.md` | 作成（Phase 1 ゲート） |
| `sessions/config-optimization/YYYYMMDD/phase-2-analysis/01_gate.md` | 作成（Phase 2 ゲート） |
| `sessions/config-optimization/YYYYMMDD/phase-3-proposal/01_gate.md` | 作成（Phase 3 ゲート） |
| `sessions/config-optimization/YYYYMMDD/phase-N-xxx/set-1/01_tasks.md` | 作成（ワーカーへのタスク指示） |
| `sessions/config-optimization/YYYYMMDD/phase-N-xxx/set-1/06_eval_report.md` | 読み取りのみ（知見集約の入力） |
| `sessions/config-optimization/YYYYMMDD/phase-N-xxx/set-1/07_issues.md` | 読み取り＋転記済みマーク追記（課題集約） |
...
| `sessions/config-optimization/_template/phase-N-xxx/_template/07_issues.md` | ワーカーセット別課題バッファのテンプレート |
