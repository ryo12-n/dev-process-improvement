...
### 2.2 description の設計パターン

YAML フロントマターの `description` フィールドは、`user-invocable` の値に応じて設計方針が異なる。

#### `user-invocable: true` の場合

ユーザーが `/` コマンドで起動先を選択する際に表示される。**何をするスキルか**が一目で分かる動詞始まりの説明を書く。
...
# 悪い例
description: 'L1マネージャーのルール定義'     # 何をするか不明
description: 'マネージャー'                     # 情報不足
```

#### `user-invocable: false` の場合

他のスキルやエージェントからオンデマンドで参照されるポリシー・チェックリスト等。description には「**何の情報を持つか**」+「**どの文脈で参照するか**」の2要素を含める。

```yaml
# 良い例（2要素パターン: [情報の内容] + [参照文脈]）
description: 'ルール変更を含む施策で、連動更新の漏れを防ぐための標準チェックリスト。L1が02_tasks.md作成時、L2がコミット前に参照する。'
description: 'マネージャーセッション（l1, triage, ...）に共通する運用パターンを定義するポリシー'

# 悪い例
description: 'ルール変更チェックリスト'         # 参照文脈が不明
description: '共通ポリシー'                     # 情報内容が不明
description: 'チェックリストです'               # 両方不明
```

### 2.3 必須セクション（ロール定義フォーマットガイド準拠）
...
SKILL.md の末尾に「関連ファイル一覧」セクションを設け、本スキルファイルの内容を変更した場合に連動更新が必要になる可能性があるファイルをテーブル形式で列挙する。
...
| ファイル | 連動更新の内容 |
|---------|-------------|
| `<パス>` | <どのような変更時に連動更新が必要か> |
```

このセクションは `rule-change-checklist` §1（関連ファイル一覧の確認）と連携し、ルール変更時の連動更新漏れを防止する。

> **注**: 本施策ではルール定義のみを行う。既存の全スキル・エージェント定義への「関連ファイル一覧」セクションの実展開は別施策として計画する。
...
エージェント定義（`agents/*.md`）の末尾にも「関連ファイル一覧」セクションを設ける。SKILL.md と同じテーブル形式を使用する。
...
> **注**: 本施策ではルール定義のみを行う。既存の全エージェント定義への実展開は別施策として計画する。

### 3.7 エージェント管理ガイドライン

Claude Code では `.claude/agents`（グローバル）と `.claude/skills/<skill>/agents`（スキル内）の2箇所にエージェント定義を配置できる。以下のガイドラインに従い使い分けること。

#### 配置先の判断基準

| 条件 | 配置先 | 例 |
|------|--------|-----|
| 特定のマネージャースキルからのみ起動される | `.claude/skills/<skill>/agents/` | l2-worker, triage-worker |
| 複数のマネージャースキルから共通で起動される | `.claude/agents/` | （現時点では該当なし） |
| スキルに紐づかない汎用エージェント | `.claude/agents/` | （現時点では該当なし） |

現在の本リポジトリでは、すべてのエージェントが特定のマネージャースキルに1対1で紐づいているため、`.claude/skills/<skill>/agents/` に配置している。

#### スコープ設計のベストプラクティス

- **単一責務**: 1エージェント = 1ロール（worker / evaluator / plan-worker 等）。複数ロールを1つのエージェントに混在させない
- **マネージャーとの対称性**: エージェントの入出力ファイルはマネージャーの「成果物確認観点」と一致させる
- **独立コンテキスト**: エージェントは独立したコンテキストウィンドウで動作するため、必要な情報は起動時にすべて渡す（マネージャーのコンテキストを暗黙に参照できない）

#### プロンプト構成のベストプラクティス

- **YAML フロントマター**: `name` と `description` を含める。`name` はケバブケース（§3.4 準拠）
- **必須セクション**: §3.1 の6セクション（あなたの役割・作業フロー・やること・やらないこと・担当ファイル・停止ルール）を含める
- **壁打ちテンプレート**: 壁打ちフェーズのフォーマットをエージェント定義内に含めることで、ワーカーが正しい形式で記録できるようにする
- **停止ルール**: コンテキストの浪費を防ぐため、停止条件と起票フォーマットを明示する

#### 新セッションタイプ追加時の設計指針

1. マネージャースキル（SKILL.md）を先に設計し、オーケストレーションフローを確定させる
2. フローの各ステップに対応するエージェント（worker / evaluator）を設計する
3. ペアリングの対称性（§3.2）を確認する
4. §5 の既存セッションタイプ一覧に追加する
...
|----------------|---------------------|----------------------|
| L1/L2 施策フロー | `.claude/skills/l1-manager/` | `agents/l2-plan-worker.md`, `agents/l2-worker.md`, `agents/l2-evaluator.md` |
...
| セッション一貫性チェックフロー | `.claude/skills/session-consistency-manager/` | `agents/sc-collection-worker.md`, `agents/sc-analysis-worker.md`, `agents/sc-proposal-worker.md`, `agents/sc-evaluator.md` |
...
- [ ] `.claude/skills/session-lifecycle-policy/SKILL.md` の §2.1 ペアリング構造テーブルに新しい実施者/評価者/オーケストレーターの組み合わせが追加されている
- [ ] 変更対象ファイルの「関連ファイル一覧」セクションに、新セッションタイプに関連するファイルが追記されている
