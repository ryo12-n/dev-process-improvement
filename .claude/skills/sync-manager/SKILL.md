...
セッション開始時にユーザーから以下の形式で入力が来る。

```
sync: <target-repo> [<direction>]
```

| パラメータ | 必須 | 説明 | 例 |
|-----------|------|------|-----|
| `target-repo` | 必須 | 同期先リポジトリ名 | `ai-driven-dev-patterns` |
| `direction` | 任意 | 同期方向（省略時は `origin-to-upstream`） | `origin-to-upstream` / `upstream-to-origin` |
