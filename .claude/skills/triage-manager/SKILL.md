...
事前調査の結果をもとに、以下の2ステップでワーカーセット数を決定する。マネージャーは常に裁量を持つ。
...
| 走査ボリューム | 推奨セット数 | 備考 |
|-------------|------------|------|
| ≤ 20件 | 1セット | 前回からの間隔が短い場合は TG-008 等のスキップも検討 |
| 21〜50件 | 2セット | 重量タスク（TG-001, TG-005）を別セットに分離 |
| 51件以上 | 3セット | TGタスクドメイン別に分散（inbox / backlog+CSV / initiative+rules） |
