...
7. **外部リポジトリの PR 作成**: `00_proposal.md` の「外部リポジトリ」セクションに記載されたリポジトリがある場合、各リポジトリで `cd <path> && gh pr create` により施策ブランチから `main` への PR を作成する
8. **PR 作成**: `gh pr create` で施策ブランチから `main` への PR を作成する
