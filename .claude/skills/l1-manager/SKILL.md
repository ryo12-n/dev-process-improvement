...
3. 計画作成（01_plan.md）
4. タスク作成（02_tasks.md）
5. Phase A→B チェック記録（08_gate_review.md）
6. L2-worker ディスパッチ
7. L2-worker 成果物確認 + Phase B→C チェック記録（08_gate_review.md）
8. L2-evaluator ディスパッチ
9. Phase C→D チェック記録（08_gate_review.md）
10. ゲート判定（08_gate_review.md）

### パターン2: 継続（`施策: <パス>`）

1. フォルダ状態確認・動作決定
2. 該当フェーズチェック記録（08_gate_review.md — 動作決定に応じた Phase チェックを記録）
3. L2-worker ディスパッチ（または L2-evaluator ディスパッチ、またはゲート判定）
4. L2 成果物確認 + 該当フェーズチェック記録（08_gate_review.md）
5. 次ステップ実行（evaluator 起動 or ゲート判定 or クローズ手順）
...
確認結果は `08_gate_review.md` の Phase B→C チェックセクションに記録すること。
