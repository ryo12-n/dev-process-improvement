...
2. `02b_dispatch_log.md` の Worker Set 一覧テーブルの施策名を記入し、初期化する
3. `09_cost_record.md` の施策名・複雑度・タスク数を記入し、初期化する
...
8. `08_gate_review.md` — L2のレポートを受けてのゲート判定（施策全体コスト集計を `09_cost_record.md` から転記し、`08_gate_review.md` の施策全体コスト集計テーブルに記入する）
...
- 完了条件: チェックリストの全7項目（確認済み / 修正済み / 該当なし）の結果が `03_work_log.md` に記録されている
- 優先度: 高（コミット前に完了すること）
...
**ディスパッチログ記録**: L2 の起動時に `02b_dispatch_log.md` に起動エントリ（タイムスタンプ・指示内容・完了条件）を追記する。L2 の完了時に完了エントリ（結果サマリ・コストデータ: input_tokens/output_tokens）を追記する。
...
確認結果は `08_gate_review.md` の Phase 4→5 チェックセクションに記録すること。フェーズコスト小計を `09_cost_record.md` に記録し、`08_gate_review.md` のフェーズコスト小計行に転記する。
...
| `sessions/initiatives/_template/02b_dispatch_log.md` | ディスパッチログテンプレートの構成に影響する変更の場合 |
| `sessions/initiatives/_template/09_cost_record.md` | コスト記録テンプレートの構成に影響する変更の場合 |
