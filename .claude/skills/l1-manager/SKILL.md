...
`02_tasks.md` の「参照ドメインエージェント」セクションにエージェントが記載されている場合は、L2 への指示に参照先エージェントのパスと参照理由を含める（`manager-common-policy` §10.3 参照）。ドメインエージェント参照は任意であり、記載が「なし」の場合はこの項目を省略してよい。
