...
3. 計画作成（02_plan.md）
4. Phase 1→2 チェック記録（05_gate_review.md）
5. L2-plan-worker ディスパッチ（phase-1-planning/set-N/08_task_division.md 作成）
6. L2-plan-worker 成果物確認 + Phase 2→3 チェック記録（05_gate_review.md）
7. L2-plan-evaluator ディスパッチ
8. Phase 3→4 チェック記録（05_gate_review.md）
9. タスク作成（03_tasks.md — phase-1-planning/set-N/08_task_division.md を参考に作成）
10. L2-worker ディスパッチ（逐次 or 並列 — `phase-1-planning/set-N/08_task_division.md` の Wave 割当に基づき判断）
    - 並列の場合: set ディレクトリ作成 → Wave 方式で並列起動 → 完了後統合
11. L2-worker 成果物確認 + Phase 4→5 チェック記録（05_gate_review.md）
12. L2-evaluator ディスパッチ
13. Phase 5→6 チェック記録（05_gate_review.md）
14. ゲート判定（05_gate_review.md）

### パターン2: 継続（`施策: <パス>`）

1. フォルダ状態確認・動作決定
2. 該当フェーズチェック記録（05_gate_review.md — 動作決定に応じた Phase チェックを記録）
3. 該当フェーズのディスパッチ（plan-worker / plan-evaluator / worker / evaluator / ゲート判定）
4. L2 成果物確認 + 該当フェーズチェック記録（05_gate_review.md）
...
### Phase 3: 計画評価・タスク作成
4. L2-plan-evaluator を起動し、`phase-1-planning/set-N/08_task_division.md` の品質を評価させる
5. `03_tasks.md` — `phase-1-planning/set-N/08_task_division.md` を参考に L2 に渡すタスクリスト（チェックボックス形式）を作成する
...
8. `05_gate_review.md` — L2のレポートを受けてのゲート判定（施策全体コスト集計を `06_cost_record.md` から転記し、`05_gate_review.md` の施策全体コスト集計テーブルに記入する）
...
Phase 1 完了後（`01_proposal.md`・`02_plan.md` 作成済み）に L2-plan-worker を起動する。
...
確認結果は `05_gate_review.md` の Phase 2→3 チェックセクションに記録すること。
...
- **評価対象**: `phase-1-planning/set-N/08_task_division.md`、`phase-1-planning/set-N/03_worker_report.md`
- **評価基準**: `02_plan.md` の成功基準
- **完了定義**: `phase-1-planning/set-N/06_eval_report.md` に評価結果が記載されている

### Phase 3→4: 03_tasks.md の作成

L2-plan-evaluator の評価完了後、`phase-1-planning/set-N/08_task_division.md` を参考に `03_tasks.md` を作成する。`phase-1-planning/set-N/08_task_division.md` のタスク割当・Wave割当・ドメインエージェント推奨を取り込みつつ、L1 の判断で調整する。
...
確認結果は `05_gate_review.md` の Phase 4→5 チェックセクションに記録すること。フェーズコスト小計を `06_cost_record.md` に記録し、`05_gate_review.md` のフェーズコスト小計行に転記する。
...
1. L2 が停止したら壁打ちフェーズの記録ファイルを読む
   ...
   - L2-plan-evaluator: `phase-1-planning/set-N/04_eval_plan.md` の「壁打ちフェーズ」セクション
