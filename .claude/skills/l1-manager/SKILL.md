...
### パターン3: プロジェクトからの施策開始（`project: <プロジェクト名>`）

1. プロジェクト todo.md 読み込み・未着手施策の一覧提示
2. ユーザーの施策選択待ち
3. 選択された施策の開始（パターン1 のステップ1〜に合流）

---

## セッション開始時の入力解釈

セッション開始時にユーザーから以下3パターンの入力が来る。それぞれに応じて動作を決定する。
...
### パターン3: `project: <プロジェクト名>`

プロジェクトから次の施策を開始する。`projects/<プロジェクト名>/todo.md` を読み、未着手の施策一覧をユーザーに提示する。

1. `projects/<プロジェクト名>/todo.md` を読み込む
2. ステータスが「未着手」の施策を一覧表示する
3. ユーザーが施策を選択したら、以下のいずれかで開始する:
   - `backlog/entries/` に対応するエントリがある場合 → パターン1 のフローに合流
   - backlog エントリがない場合 → `projects/<プロジェクト名>/plan.md` の施策情報を参照し、壁打ちフェーズから新施策を開始する。`01_proposal.md` の「プロジェクト紐づけ」にプロジェクト名とパスを記入する
4. 施策開始後、`projects/<プロジェクト名>/todo.md` の該当施策のステータスを「進行中」に更新する
...
タスク設計時に、ワーカーが繰り返し適用する定型テンプレート（`.claude/templates/` 配下）がある場合は、L1 がタスク作成段階でテンプレートを事前適用することを検討する。これにより、ワーカーの作業効率が向上し、テンプレート適用の抜け漏れを防止できる。

- **適用例**: 評価フレームワークテンプレートを `phase-2-execution/set-N/04_eval_plan.md` に事前展開し、ワーカーには記入のみを委任する
- **判断基準**: テンプレートの適用先が明確で、ワーカー間で適用方法にばらつきが生じうる場合に有効
- **注意**: 強制ではない。タスクの性質に応じて L1 が判断する
...
- やること: 施策クローズ時に以下の3ファイルを更新する
  ...
  3. `projects/<name>/plan.md` の施策一覧テーブルのステータスを更新する。ブロック・スコープ変更等があれば方針・制約セクションに注記する
- 完了条件: todo.md のステータスが更新済み、knowledge.md に知見が追記済み（該当なしなら「なし」記載）、plan.md のステータスが更新済み
- 優先度: 高（施策クローズ前に完了すること）
- 適用対象環境: CLI / SDK 横断
