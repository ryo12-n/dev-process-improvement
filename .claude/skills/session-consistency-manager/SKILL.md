...
1. セッションフォルダを作成する: `sessions/session-consistency/_template/` 直下のファイル（00〜04）のみを `sessions/session-consistency/YYYYMMDD/` にコピーし、`sessions/session-consistency/YYYYMMDD/workers/` は空ディレクトリとして作成する（`_template/workers/_template/` はコピーしない）
2. `00_pre_investigation.md` の穴埋めを実施する（現状把握）
   ...
   - `triage-standard-policy` §1.2 適用マトリクスの現状
...
| SC-003 | Worker ↔ Evaluator 対称性 | session-flow-policy §3.2 + triage-standard-policy §2.3 のペアリング対称性要件（壁打ち・計画・レポート・知見記録・課題起票・停止ルール）が全ペアで満たされているか |
...
| SC-006 | triage-standard-policy 適用マトリクス整合 | §1.2 の全セッション × ステージのマトリクス記載とロール定義の一致 |
...
1. **セット作成**: `sessions/session-consistency/_template/workers/_template/` を `workers/set-1/` にコピーする
2. **タスク作成**: `workers/set-1/01_tasks.md` に SC-001〜SC-008 の情報収集タスクを記載
...
1. **セット作成**: `sessions/session-consistency/_template/workers/_template/` を `workers/set-2/` にコピーする
2. **タスク作成**: `workers/set-2/01_tasks.md` に SC-001〜SC-008 の現状分析タスクを記載
...
1. **セット作成**: `sessions/session-consistency/_template/workers/_template/` を `workers/set-3/` にコピーする
2. **タスク作成**: `workers/set-3/01_tasks.md` に SC-001〜SC-008 の提案作成タスクを記載
...
1. `sessions/session-consistency/_template/workers/_template/` を `sessions/session-consistency/YYYYMMDD/workers/set-N/` にコピーする（`_template/workers/_template/` から直接コピーする）
...
| `sessions/session-consistency/YYYYMMDD/workers/set-N/01_tasks.md` | 作成（ワーカーへのタスク指示） |
| `sessions/session-consistency/YYYYMMDD/workers/set-N/06_eval_report.md` | 読み取りのみ（知見集約の入力） |
| `sessions/session-consistency/YYYYMMDD/workers/set-N/07_issues.md` | 読み取り＋転記済みマーク追記（課題集約） |
...
| `sessions/session-consistency/_template/workers/_template/07_issues.md` | ワーカーセット別課題バッファのテンプレート |
