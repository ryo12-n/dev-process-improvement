...
1. セッションフォルダを作成する: `sessions/session-consistency/_template/` 直下のファイル（00〜04）と `phase-N-xxx/01_gate.md` を `sessions/session-consistency/YYYYMMDD/` にコピーする（各 `phase-N-xxx/workers/_template/` はコピーしない）
...
1. **セット作成**: `sessions/session-consistency/_template/phase-1-collection/workers/_template/` を `phase-1-collection/workers/set-1/` にコピーする
2. **タスク作成**: `phase-1-collection/workers/set-1/01_tasks.md` に SC-001〜SC-008 の情報収集タスクを記載
...
1. **セット作成**: `sessions/session-consistency/_template/phase-2-analysis/workers/_template/` を `phase-2-analysis/workers/set-1/` にコピーする
2. **タスク作成**: `phase-2-analysis/workers/set-1/01_tasks.md` に SC-001〜SC-008 の現状分析タスクを記載
...
1. **セット作成**: `sessions/session-consistency/_template/phase-3-proposal/workers/_template/` を `phase-3-proposal/workers/set-1/` にコピーする
2. **タスク作成**: `phase-3-proposal/workers/set-1/01_tasks.md` に SC-001〜SC-008 の提案作成タスクを記載
...
1. `sessions/session-consistency/_template/phase-N-xxx/workers/_template/` を `sessions/session-consistency/YYYYMMDD/phase-N-xxx/workers/set-1/` にコピーする（該当フェーズの `_template/phase-N-xxx/workers/_template/` から直接コピーする）
2. `phase-N-xxx/workers/set-1/01_tasks.md` に該当フェーズのタスクを記入する
...
1. 各フェーズの `phase-N-xxx/workers/set-1/04_scan_report.md` の分析結果をフェーズ別にマージする
2. 各フェーズの `phase-N-xxx/workers/set-1/06_eval_report.md` の評価結果を「Phase サマリ」セクションに反映する
...
| `sessions/session-consistency/YYYYMMDD/phase-N-xxx/workers/set-1/01_tasks.md` | 作成（ワーカーへのタスク指示） |
| `sessions/session-consistency/YYYYMMDD/phase-N-xxx/workers/set-1/06_eval_report.md` | 読み取りのみ（知見集約の入力） |
| `sessions/session-consistency/YYYYMMDD/phase-N-xxx/workers/set-1/07_issues.md` | 読み取り＋転記済みマーク追記（課題集約） |
...
- ワーカーの成果物（`phase-N-xxx/workers/set-1/03_work_log.md`、`04_scan_report.md`）は直接編集しない（読み取りのみ）
- 評価者の成果物（`phase-N-xxx/workers/set-1/05_eval_plan.md`、`06_eval_report.md`）は直接編集しない（読み取りのみ）
...
| `sessions/session-consistency/_template/phase-N-xxx/workers/_template/07_issues.md` | ワーカーセット別課題バッファのテンプレート |
