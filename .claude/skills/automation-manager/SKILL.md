...
1. セッションフォルダを作成する: `sessions/automation/_template/` 直下のファイル（00〜04）のみを `sessions/automation/YYYYMMDD/` にコピーし、`sessions/automation/YYYYMMDD/phase-1-scan/` は空ディレクトリとして作成する（`_template/phase-1-scan/_template/` はコピーしない）
2. `00_pre_investigation.md` の穴埋めを実施する（現状把握）
   ...
   - `プロセス改善_課題管理.csv`（.md から自動生成）の関連課題を確認する
...
7. レポートに基づくアクションを実施する:
   - 新規候補登録 → `automation-candidates/entries/AC-XXX.md` を作成し、`automation-candidates/automation-candidates.csv` に行を追加
   - ステータス変更 → `automation-candidates/entries/AC-XXX.md` のステータスを更新し、`automation-candidates/automation-candidates.csv` の該当行を更新
   - 優先度スコア変更 → `automation-candidates/entries/AC-XXX.md` の優先度を更新し、`automation-candidates/automation-candidates.csv` の該当行を更新
   - backlog 起票提案 → `backlog/entries/<施策名>.md` を作成し、`backlog/backlog.csv` に行を追加
   - 知見ルーティング → `inbox/*.md` にアクション可能な洞察を起票
   - 課題 CSV 転記 → `プロセス改善_課題管理.csv` に転記し、`issues/entries/ISS-XXX.md` を作成
...
1. `sessions/automation/_template/phase-1-scan/_template/` を `sessions/automation/YYYYMMDD/phase-1-scan/set-N/` にコピーする（セッションフォルダ内の `phase-1-scan/_template/` ではなく、リポジトリの `_template/phase-1-scan/_template/` から直接コピーする）
2. `phase-1-scan/set-N/01_tasks.md` に割り当てる AT タスクを記入する
...
1. 各 `phase-1-scan/set-N/04_scan_report.md` の走査結果を AT タスク別にマージする
...
- 各 `phase-1-scan/set-N/04_scan_report.md` の「走査中の知見」
...
| 明確な課題（対応が必要） | `issues/entries/ISS-XXX.md` を作成して転記（CSV は自動生成） | 「候補管理プロセスの改善が必要」 |
...
| `sessions/automation/YYYYMMDD/phase-1-scan/set-N/01_tasks.md` | 作成（ワーカーへのタスク指示） |
| `sessions/automation/YYYYMMDD/phase-1-scan/set-N/06_eval_report.md` | 読み取りのみ（知見集約の入力） |
| `sessions/automation/YYYYMMDD/phase-1-scan/set-N/07_issues.md` | 読み取り＋転記済みマーク追記（課題集約） |
| `automation-candidates/automation-candidates.csv` | 更新（新規候補の行追加・既存候補のステータス/優先度更新） |
| `automation-candidates/entries/AC-XXX.md` | 作成（新規候補）・更新（ステータス/優先度変更） |
| `backlog/entries/*.md` | 作成（backlog 起票提案時） |
...
- `03_report.md` を作成し、レポートに基づくアクションを実施する（automation-candidates CSV/entries の新規登録・ステータス更新・優先度更新、backlog/entries/ への起票、inbox/ への知見起票、プロセス改善_課題管理.csv への課題転記）
...
- **候補管理の変更は PR 内で実施する**: 新規候補登録・優先度変更・ステータス更新はアクション実施ステップ（Step 7）で適用し、PR に含めてユーザーが PR レビューで確認・承認する
...
| `sessions/automation/_template/phase-1-scan/_template/07_issues.md` | ワーカーセット別課題バッファのテンプレート |
